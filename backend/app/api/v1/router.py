...
from app.application.nl2sql_service import (
    process_nl_query,
    process_nl_query_stream,
    execute_prepared_sql_response,
)
...
from app.api.v1.schemas import (
    ...
    QueryLogOut, UserOut, SemanticTermOut, StatsOut, GuardrailReportOut, QueryResultOut, SQLExecuteRequest
)
from app.domain.entities import NL2SQLResponse, GuardrailReport, GuardrailSeverity
...
async def run_nl_query(req: NLQueryRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        resp = await process_nl_query(req.query, db, req.user_id, req.manual_approval)
...
async def run_nl_query_stream(req: NLQueryRequest, db: AsyncSession = Depends(get_db)):
    ...
    async def event_generator():
        try:
            async for event in process_nl_query_stream(req.query, db, req.user_id, req.manual_approval):
...
@router.post("/query/execute", response_model=NLQueryResponse, tags=["NL2SQL"])
async def execute_prepared_sql(req: SQLExecuteRequest, db: AsyncSession = Depends(get_db)):
    """Execute previously approved SQL generated by AI."""
    try:
        prepared = NL2SQLResponse(
            ...
            guardrail=GuardrailReport(severity=GuardrailSeverity.OK),
            result=None,
        ...
        resp = await execute_prepared_sql_response(prepared, db, req.user_id)
    except DriveeryException as e:
        raise HTTPException(status_code=400, detail={"error": e.message, "code": e.code})
    except Exception as e:
        raise HTTPException(status_code=500, detail={"error": str(e), "code": "INTERNAL"})
...
async def save_report(req: SaveReportRequest, db: AsyncSession = Depends(get_db)):
    ...
    user_id = req.user_id
    if user_id is None:
        first_user = await db.scalar(select(User.id).order_by(User.id.asc()).limit(1))
        if first_user is None:
            raise HTTPException(
                status_code=400,
                detail={"error": "Не найден пользователь для сохранения отчета", "code": "USER_NOT_FOUND"},
            )
        user_id = first_user
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail={"error": "Не удалось сохранить отчет в базу данных", "code": "REPORT_SAVE_FAILED"},
        )
    return report
